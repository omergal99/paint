...
full light-theme report is retained as internal engineering evidence under
`docs-internal/retro/reports/` and is intentionally excluded from the public
GitHub release.

| Point | Result | Example / how to verify | Remaining gap |
|---:|---|---|---|
| 1. Persistent retro memory | Done | Inspect the local-only `docs-internal/retro/index.json` and `memory/`. | Keep adding only non-sensitive, evidence-backed learnings. |
...
| 6. Guidance update | Done | Review `.skills/architecture.md`, `.skills/validation.md`, and the local-only `docs-internal/retro/skills/release-gate.md`. | Apply the round template to future work. |
